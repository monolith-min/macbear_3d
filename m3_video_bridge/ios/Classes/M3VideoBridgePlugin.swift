  ...
  private func setupVideo(textureId: Int, assetPath: String) {
    ...
    if lowerPath.hasPrefix("http://") || lowerPath.hasPrefix("https://") {
      guard let networkUrl = URL(string: assetPath) else {
        print("--- M3VideoBridge iOS: ERROR: Invalid network URL: \(assetPath)")
        return
      }
      url = networkUrl
      print("--- M3VideoBridge iOS (Network): \(url)")
    } else {
      let assetKey = registrar?.lookupKey(forAsset: assetPath) ?? assetPath
      // Construct absolute path from main bundle for iOS
      let mainBundle = Bundle.main
      if let finalPath = mainBundle.path(forResource: assetKey, ofType: nil) {
        url = URL(fileURLWithPath: finalPath)
        print("--- M3VideoBridge iOS (Asset): \(finalPath)")
      } else if FileManager.default.fileExists(atPath: assetKey) {
        // Try fallback if it's already an absolute path
        url = URL(fileURLWithPath: assetKey)
      } else {
        print("--- M3VideoBridge: File NOT found for assetKey: \(assetKey)")
        return 
      }
    }
...
class M3VideoPlayer: NSObject {
  ...
  init(url: URL) {
    super.init()
    print("--- M3VideoBridge asset: \(url.path)")
